import { useSelector} from 'react-redux';

function AgreedmentPage() {
    const user = useSelector(state => state.user.user)

    const getDate = () => {
        const month = ['січня', 'лютого', "березня", "квітня", "травня", "червня", "липня", "серпня", "вересня", "жовтня", "листопада", "грудня"]

      return {day: new Date().getDate(), month: month[new Date().getMonth()] , year: new Date().getFullYear()}
    }

    const getFullName = () =>{
        return user.firstname + ' ' + user.lastname + ' ' + user.thirdname
    }

    return (
        <>
                        <div className="header-block">
                            <div className="block-1">
                                <a>СТОМАТОЛОГІЧНИЙ КАБІНЕТ</a>
                                <a>ФОП Сабана Іллі Любомировича</a>
                            </div>
                            <div className="block-2">Медична карта № ___________</div>
                        </div>
                        <div className="logo">
                            <img
                                src="data:image/png;base64,"/>
                        </div>

                        <h4 className="title_name">Інформована згода <br/>на попередній огляд та консультацію
                        </h4>

                        <div className="content">
                            <p>Я, {getFullName()} добровільно звертаюся за медичною допомогою у стоматологічну клініку «Frankolab clinic» та надаю
                                повну відому мені інформацію про стан мого здоров'я.</p>
                            <p>Я погоджуюся на проведення огляду порожнини рота, маніпуляцій та досліджень (включаючи
                                рентгенологічні) для отримання максимально повної картини стану моєї зубощелепної системи в цілому і
                                окремих зубів зокрема.</p>
                            <p>Я згоден{user.gender == 'Жінка' && 'a'} на складання попереднього плану лікування. З прейскурантом ознайомлен{user.gender == 'Жінка' ? 'на' : 'ий'}.</p>
                            <p>Я даю згоду на обробку та внесення до локальної інформаційної системи моїх персональних даних, у т.ч.
                                щодо стану здоров’я, діагнозів, призначеного лікування, його перебігу.</p>
                        </div>

                        <div className="signature-user">
                            <a className="data">«<span className='date-line'>{getDate().day}</span>»<span className='date-line'>{getDate().month}</span> {getDate().year} р.</a>
                            <div className="sign">
                                <div className="sign-block">
                                    <a className="sign-value sign-value-1"></a>
                                    <div className="sign-desc">підпис</div>
                                </div>
                                <div className="sign-block">
                                    <a className="sign-value">{user.firstname + ' ' + user.lastname}</a>
                                    <div className="sign-desc">прізвище, ініціали пацієнта</div>
                                </div>
                            </div>
                        </div>
                        <h6 className="signature-doc-title">Згоду пацієнт прочитав і підписав у моїй присутності</h6>
                        <div className="signature-user signature-doc">
                            <a className="data">Лікуючий лікар</a>
                            <div className="sign">
                                <div className="sign-block">
                                    <a className="sign-value sign-value-1"></a>
                                    <div className="sign-desc">підпис</div>
                                </div>
                                <div className="sign-block">
                                    <a className="sign-value sign-value-1"></a>
                                    <div className="sign-desc">прізвище, ініціали лікаря</div>
                                </div>
                            </div>
                        </div>

        </>
    );
}

export default AgreedmentPage;
